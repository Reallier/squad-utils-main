...
type PromtailHandler struct {
	// Image 所用镜像
	Image string
	// 容器名字,格式为 {UUID}-promtail
	Name string
	// Env 启动环境变量
...
func NewPromtailHandlerFromEvent(event events.Message) (*PromtailHandler, error) {
	...
	// 这一步可以检查 A2S 容器在不在
	promtailContainerName := fmt.Sprintf("%s-promtail", handler.UUID)
	handler.Name = promtailContainerName
	log.Debug().Str("name", handler.Name).Msg("期望的日志上报容器名字")
	promtailContainer, err := GetContainerByName(handler.Name)
	if err != nil {
		// 发生一个错误,看看是不是不存在
		if client.IsErrNotFound(err) {
			log.Info().Err(err).Str("name", handler.Name).Msg("日志上报容器不存在,标记重建")
			handler.ReCreate = true
		} else {
			// 其他错误,退出
			log.Err(err).Str("name", handler.Name).Msg("获取日志上报容器信息失败")
			return nil, err
		}
	} else {
		// 无错误,继续
		handler.ContainerID = promtailContainer.ID
	}
	// 例如 /var/lib/pterodactyl/volumes/{UUID}/Steam/logs/steam.yaml
	handler.ConfigFilePath = path.Join(config.SQConfig.Volumes.Pter, handler.UUID, "Steam", "logs", "steam.yaml")
	// 先看文件在不在
	if _, err := os.Stat(handler.ConfigFilePath); errors.Is(err, os.ErrNotExist) {
		// 不存在应当报错退出
		log.Err(err).Str("name", handler.Name).Msg("日志上报配置文件不存在")
		return nil, err
	}
	// 读取文件计算 SHA256
	// 用于后续判断是否需要重建容器
	...
	if err != nil {
		log.Err(err).Str("name", handler.Name).Msg("日志上报配置文件打开出错")
		return nil, err
	}
	defer f.Close()
	...
	if _, err := io.Copy(h, f); err != nil {
		log.Err(err).Str("name", handler.Name).Msg("日志上报配置文件哈希计算出错")
		return nil, err
	}
...
func (a *PromtailHandler) Deploy() error {
...
func (a *PromtailHandler) Pull(force bool) error {
	ctx := context.Background()
	log.Info().Str("name", a.Name).Msg("检查镜像更新")
	pull, err := config.DockerCli.ImagePull(ctx, config.SQConfig.StartupImages.Promtail, types.ImagePullOptions{})
	if err != nil {
		log.Err(err).Str("name", a.Name).Msg("拉取镜像出问题")
		return err
	}
	defer pull.Close()
	buf := make([]byte, 4096)
...
// Create 创建容器
func (a *PromtailHandler) Create() error {
	ctx := context.Background()
	...
	containerConfig := &container.Config{
		...
		Env:     nil,
		Image:   config.SQConfig.StartupImages.Promtail,
		Labels: map[string]string{
			"sq.confighash": a.ConfigHash,
		},
		Cmd: []string{"-config.file", a.ConfigFilePath},
	}
...
// Remove 删除容器
func (a *PromtailHandler) Remove() error {
...
func (a *PromtailHandler) Start() error {
